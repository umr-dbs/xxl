 ...
 * 
 */
public class MVBT extends BPlusTree {
	...
	public MVBT(int blockSize, float minCapRatio, float e) {
		...
		this.keyDomainMinValue = MVBT.DEAFUALT_KEYDOMAIN_MINVALUE;
	}
	 ...
	 */	
	public MVBT initialize(	Function getKey,
...
//								};
	 ...
	 * @return the initialized <tt>MVBTree</tt>.
	 */	
	public MVBT initialize(	final Function getKey,
...
//			};
		...
		return new MeasuredConverter() {
			public Object read(DataInput input, Object object) throws IOException {
				...
				MVRegion mvReg= createMVRegion(lifespan.beginVersion(), lifespan.endVersion(), 
													minBound, maxBound);
				Object rootId= MVBT.this.container().objectIdConverter().read(input,null);
	...
	public Object remove(Version removeVersion, Object data) {
		...
		Iterator it = ((MVBT.Node)node(path)).iterator();
...
//		return treequery(createMVRegion(version, version, min, max));
   ...
   * 			when the tree is in duplicate mode 
	...
	protected IndexEntry chooseEntry(MVSeparator mvSeparator, Iterator entries) {
		List currentEntries = new ArrayList(Math.max(MVBT.this.B_LeafNode, MVBT.this.B_IndexNode));
...
//			return chooseEntry(mvSeparator, iterator);
		 ...
		 *  {@link MVBT#keyRangeQuery(Comparable, Comparable, xxl.core.indexStructures.MVBT.Version)} query.
	...
	public class Root {
		...
		public Node get(boolean unfix) {
			return (Node) MVBT.this.container().get(rootId, unfix);
		}
	...
	protected class MVBTreeQueryCursor extends xxl.core.indexStructures.QueryCursor {
		...
		public MVBTreeQueryCursor(IndexEntry indexEntry, MVRegion nodeRegion, MVRegion queryRegion) {
			super(MVBT.this, indexEntry, queryRegion, 0);
...
firstFor:	for (int i = 0; i < candidates.size(); i++ ){
		 ...
		 * @see MVBT.Node#referenceLeafQuery(xxl.core.indexStructures.MVBT.MVRegion, xxl.core.indexStructures.MVBT.MVRegion)
		 * @param queryRegion
		 * @param node
		 * @param nodeSeparator
		 * @return
